...
def _sync_weights_noop(nodes, eta=0.05):
    pass

def run_isolation_trace(total_steps=10000, checkpoint_every=500):
    engine_mod.sync_weights = _sync_weights_noop
    ...
    print("\n--- Summary (sync_weights DISABLED) ---")
    counts = [len(v) for _, v, _ in history]
    margins_hist = [m for _, _, m in history]
    ...
    print(f"system_margin first 5: {[round(m,4) for m in margins_hist[:5]]}")
    print(f"system_margin last 5: {[round(m,4) for m in margins_hist[-5:]]}")

if __name__ == "__main__":
    run_isolation_trace(total_steps=10000, checkpoint_every=500)
