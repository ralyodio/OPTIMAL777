...
theorem OU_variance_decreases_with_theta
  ...
  apply div_lt_div_of_pos_left (sq_pos_of_pos hσ)
    (by positivity) (by linarith)

-- OU mean-reversion path bound
...
-- At stationarity: d/dx[μp] = (1/2)d²/dx²[σ²p]
...
-- Gaussian is stationary for OU process
-- p*(x) = N(0, σ²/2θ): check p* satisfies balance
...
-- Log-normal: log(S(t)) is normally distributed
...
-- Linear growth bound from Lipschitz
...
-- Unique strong solution exists (statement)
...
theorem variance_decays_with_kappa
  ...
  unfold variance_propagation
  have : Real.exp (-2 * kappa * T) < 1 := by
    apply Real.exp_lt_one_of_neg
    nlinarith
  linarith [mul_lt_iff_lt_one_right hV0 |>.mpr this]
...
-- Each domain has mean-reversion and noise parameters
...
-- Stationary variance for each domain
