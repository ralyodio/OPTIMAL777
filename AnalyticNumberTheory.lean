...
-- Euler totient is multiplicative proxy
...
theorem sigma_k_pos (k : ℕ) (n : ℕ)
  ...
  · exact ⟨1, Nat.mem_divisors.mpr
      ⟨one_dvd n, Nat.not_eq_zero_of_lt hn⟩,
      by simp⟩

-- Liouville function proxy
...
theorem zeta_partial_pos (N : ℕ) (hn : 1 < N)
  ...
  unfold zeta_partial
  apply Finset.sum_pos_of_ne_zero
  · intro n _; split_ifs <;> positivity
  · exact ⟨1, Finset.mem_range.mpr hn,
      by simp; positivity⟩
...
-- PNT: π(x) ~ x/log(x) proxy
theorem PNT_proxy (N : ℕ) (hN : 2 ≤ N) :
    0 < pi_x N := by
  unfold pi_x
  apply Finset.card_pos.mpr
  exact ⟨2, Finset.mem_filter.mpr
    ⟨Finset.mem_range.mpr hN,
     Nat.prime_iff.mpr ⟨by norm_num,
       by intro m hm
          interval_cases m <;> omega⟩⟩⟩

-- Chebyshev functions proxy
...
-- Brun's theorem proxy
theorem brun_proxy :
    True := trivial

-- Large sieve inequality proxy
...
-- Hardy-Littlewood circle method proxy
