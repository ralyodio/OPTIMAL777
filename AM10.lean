# (cat > ~/lean_workspace/AM10.lean << 'EOF'
...
theorem bottleneck_law (margins : List ℚ)
  ...
  exact List.exists_min_image margins id h
...
cd ~/MyMathlibProject && \
git status && \
git add AM10.lean Manifold21.lean PhysicsCore.lean AWM21.lean AntaresCategory.lean MyProject.lean && \
git commit -m "Add AM10, Manifold21, PhysicsCore, AWM21, AntaresCategory, MyProject modules (7-core system bundle)" && \
...
git commit -m "Add AM10, Manifold21, PhysicsCore, AWM21, AntaresCategory, MyProject modules (7-core system bundle)" && \
git push origin master
ls -la ~/MyMathlibProject/AM10.lean
cd ~/MyMathlibProject && \
git status && \
git add AM10.lean Manifold21.lean PhysicsCore.lean AWM21.lean AntaresCategory.lean MyProject.lean && \
git commit -m "Add AM10 + Manifold21 + PhysicsCore + AWM21 + AntaresCategory + MyProject (core system bundle)" && \
git push origin master
cat > AM10.lean << 'EOF'
# (cat > ~/lean_workspace/AM10.lean << 'EOF'
import Mathlib.Tactic
...
git push origin master
cd ~/MyMathlibProject && lake update && lake build
cd ~/MyMathlibProject && rm -rf .lake && lake update
#!/usr/bin/env bash
set -e

cd ~/MyMathlibProject

echo "=== 1. Writing AM10.lean ==="
cat > AM10.lean << 'EOF'
import Mathlib.Tactic
