...
theorem sa_trace_real (A : H →L[ℂ] H) (hA : ContinuousLinearMap.adjoint A = A) :
  ...
  have h : LinearMap.trace ℂ H (A : H →ₗ[ℂ] H)
      = starRingEnd ℂ (LinearMap.trace ℂ H (A : H →ₗ[ℂ] H)) := by
    conv_lhs => rw [← hA]
    exact (LinearMap.trace_conj' ℂ H (A : H →ₗ[ℂ] H)).symm
  have him := congrArg Complex.im h
  simpa using him

theorem sa_product_trace_real (A B : H →L[ℂ] H)
    (hA : ContinuousLinearMap.adjoint A = A) (hB : ContinuousLinearMap.adjoint B = B) :
    (LinearMap.trace ℂ H ((A.comp B) : H →ₗ[ℂ] H)).im = 0 := by
